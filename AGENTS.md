...
- The authenticated Supabase app nav is bottom-only and task-based: `My Game`, `Tournament`, `Archive`, `Profile`, plus admin-only `Admin`. Do not show admin setup to non-admins in nav, and keep sign-out in Profile with confirmation.
