...
- `src/features/tournament2026/components/LeaderboardSection.tsx` derives live overall, foursomes, and singles totals from hole outcomes. Keep the Scores tab visually aligned with the Archive view: flat full-width ledgers and feeds with thin dividers, not tiled dashboard cards or a `Panel` containing more panels. Secondary Scores surfaces should be collapsible while the top match state stays visible.
